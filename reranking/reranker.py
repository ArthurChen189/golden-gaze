...
parser.add_argument('-t', '--tag', help='Run tag', choices=['2022-BM25-Baseline', '2022-Trust-Pipeline', '2022-Correct-Stance'],
                    required=True)
args = parser.parse_args()
stance_path_map = {'2022-BM25-Baseline': '../initial_retrieval/output/2022/',
                   '2022-Trust-Pipeline': '../stance_detection/output/2022topics_inference/',
                   '2022-Correct-Stance': '../stance_detection/output/2022topics_inference/', }

# Load files
prediction = []
for topic_id in range(151, 201):
    file_path = f'{stance_path_map[args.tag]}{topic_id}.csv'
    dataframe = pd.read_csv(file_path)
    prediction.append(dataframe)
prediction = pd.concat(prediction, axis=0)
topic_stance = pd.read_csv('../answer_prediction/output/2022_stance_prediction.csv')
...
def combine_scores(record):
    if args.tag == '2022-BM25-Baseline':
        return record['score']
...
def fix_docno(record):
    ...
    if not re.match(r'en\.noclean\.c4-train.[0-9]+-of-[0-9]+.[0-9]+', docno):
        raise SystemExit(f'\033[docnos are not in an acceptable string format: c4-[0-9]+-[0-9]+ !\033[0m')
    docno_split = docno.split('-')
    file_number = int(docno_split[1].split('.')[1])
    json_loc = int(docno_split[3].split('.')[1])
...
prediction.to_csv(f'output/2022/{args.tag}', sep=' ', header=None, index=False)
